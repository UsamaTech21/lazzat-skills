# Ad Copy Templates

> **Rule:** Fill the `[brackets]`, run compliance, launch on a Tuesday. No exceptions.

---

## Meta — Awareness (Primary)

```
Primary text (125 char sweet spot):
Volcanic stone. Real flame. 200 grams, honestly weighed. Lazzat Grill & Shakes — now open in Brampton. Every table welcome.

Headline (40 char):
Lava stone grill. Real flame.

Description (30 char):
Halal-sourced. Open kitchen.

CTA:
See Menu
```

---

## Meta — Platter Push

```
Primary text:
One platter. Ten stories. Lava stone grilled meats, 13 sauces, honest portions — at our McVean Dr and Clarence St stores.

Headline:
Feed the whole table.

Description:
Halal-sourced. Every culture welcome.

CTA:
Order Now
```

---

## Meta — Catering Lead-Gen (B2B)

```
Primary text:
Office lunches that don't feel like office lunches. Lazzat catering: lava stone platters, 200g protein portions, dietary-inclusive (veg + vegan options). 24 hours' notice.

Headline:
Corporate catering — Brampton.

Description:
From 10 people to 200.

CTA (Instant Form):
Get a Quote
```

---

## Meta — Bowl Builder (Young Professional)

```
Primary text:
Build your bowl. Pick your protein. Pick your base. Pick your sauce. 200 grams, honestly weighed. Lava stone grilled. Under $16.

Headline:
Build your bowl at Lazzat.

Description:
High-protein. Real flame.

CTA:
Order Now
```

---

## Meta — Launch Countdown (Pre-Apr 26)

```
Primary text:
Brampton — we open Saturday, April 26. Lava stone grill. 200g honest portions. Thirteen sauces. One grill for every table.

Headline:
Opens Saturday. McVean Dr.

Description:
Halal-sourced. Every culture welcome.

CTA:
Get Directions
```

---

## Meta — Seasonal Event Template

```
Primary text:
[Event name] done right — [specific menu tie-in]. At Lazzat, [one line about the dish, no regional framing, no religious phrase]. From our lava stone to your table.

Headline:
[Event] — [dish].

Description:
Halal-sourced. Every table welcome.

CTA:
See the Menu
```

Filled example (Canada Day):
```
Primary text:
Canada Day done right — Mega Platter for a long-table summer. Four proteins. Rice. Salads. Thirteen sauces. From our lava stone to your backyard.

Headline:
Canada Day — Mega Platter.

Description:
Order 24h ahead.

CTA:
Book Catering
```

Filled example (Ramadan / Iftar — respectful, not religious):
```
Primary text:
Iftar bowls built for the community. Lava stone grilled, 200g protein, dietary-inclusive sides. Easy pickup from McVean Dr or Clarence St.

Headline:
Iftar — Lazzat Bowls.

Description:
Every table welcome.

CTA:
See the Menu
```

---

## Instagram Story / Reel Ad

```
VISUAL: [hero shot or 3-sec stone sear]
CAPTION (6 words max on screen): "Lava stone. Real flame."
CTA overlay: "See Menu"
Swipe-up / link sticker: lazzat.ca/menu
SOUND: Lazzat original sound OR cleared trending audio
DURATION: 9–15 sec
```

---

## Google Search — Responsive Search Ad (RSA)

### Campaign: Halal Grill Brampton

**Headlines (30-char max each — provide 15):**
1. Halal Grill Brampton — Lazzat
2. Lava Stone Grill Brampton
3. Open Kitchen — Real Flame
4. McVean Dr + Clarence St
5. 200g Honest Protein Bowls
6. Halal-Sourced Meats Daily
7. Built for Every Table
8. Visit Lazzat Today
9. Order Online — Lazzat
10. Every Culture Welcome
11. Lazzat Grill & Shakes
12. No Pork. No Beef. No Alcohol.
13. 13 Sauces. One Grill.
14. Catering Brampton Halal
15. Uniting Through Flavours

**Descriptions (90-char max each — provide 4):**
1. Volcanic stone. Real flame. 200g honest portions. Halal-sourced. Open kitchen. Brampton.
2. Lazzat Grill & Shakes — McVean Dr + Clarence St. See the menu and order now.
3. Halal catering in Brampton — office lunches, Iftar, Diwali, Canada Day. Book today.
4. Built for Brampton. Every table welcome. Lava stone grilled, honestly weighed.

**Path fields:**
- Path 1: Brampton
- Path 2: LazzatGrill

---

## Google PMax — Asset Group (per location)

- **Images:** 5 brand images (hero platter, bowl flat-lay, lava stone cook, storefront exterior, team shot)
- **Videos:** 3 Shorts cuts (15s each)
- **Logos:** square logo + wide-banner logo
- **Headlines (5):** same pool as RSA above
- **Long headlines (5, 90-char max):**
  1. Lava stone grill in Brampton — halal-sourced, open kitchen, every culture welcome.
  2. Lazzat Grill & Shakes — McVean Dr + Clarence St. Bowls, platters, wraps, shakes.
  3. 200 grams. Honestly weighed. Thirteen sauces. Lava stone grilled. Lazzat Brampton.
  4. Brampton's lava stone grill — no pork, no beef, no alcohol. Every table welcome.
  5. Catering from Lazzat — lava stone platters, dietary-inclusive, 24h notice.
- **Descriptions (5, 90-char max):** as RSA
- **CTAs:** See Menu / Order Now / Get Directions / Book Catering

---

## TikTok / YouTube Shorts Ad

```
HOOK (0–2s): Close-up — protein hits lava stone, steam rises.
TEXT (on-screen): "No charcoal."
SCENE 2 (2–6s): Chef Mudassar weighs 200g cube on scale.
TEXT: "200 grams. Every time."
SCENE 3 (6–10s): Bowl assembly — sauce drizzle.
TEXT: "13 sauces. One grill."
CTA (10–15s): Hero shot — logo overlay.
TEXT: "Lazzat Grill & Shakes — Brampton."
CAPTION FIRST LINE:
From volcanic stone to your table. Every culture welcome.
```

---

## Claim-Check Cheat Sheet

Before running ANY ad, confirm every line against:

| Line | GREEN if on approved-claims.md |
|---|---|
| "halal-sourced" | ✅ |
| "halal certified" | ❌ (pending certification) |
| "no pork, no beef, no alcohol" | ✅ |
| "lava stone grilled" | ✅ |
| "best in Brampton" | ❌ (superlative forbidden) |
| "authentic Pakistani" | ❌ (regional framing banned) |
| "healthy" (without claim backup) | ❌ (superlative; use "honestly weighed 200g" instead) |
| "200 grams honestly weighed" | ✅ |
| "Maple Lodge Farms" | ✅ (with PR approval first) |
| "Chef Mudassar" | ✅ |
| "Professor Parveen validated" | ✅ (only when blog/long-form; not paid-ad short copy) |
| "Uniting Through Flavours" | ✅ (primary tagline) |
| "One grill. Every story." | ✅ (secondary tagline) |

---

## Compliance Block (append to every ad before Publish)

```
COMPLIANCE BLOCK
- Pillar: [1 of 6]
- Menu items: [or "N/A"]
- Approved claims used: [list]
- Tagline used: [primary/secondary/none]
- Banned words scan: ✅ pass
- Regional framing: ✅ none
- Approver: Usama (routine) / Sir Waqas (strategic)
- Launch date: [Tuesday, YYYY-MM-DD]
```
